...
public:
    ...
    bool save_rdb();
    bool load_rdb();
    void set_rdb_file(const std::string& filename) { rdb_filename = filename; }
...
private:
    ...
    // RDB I/O 辅助
    static void write_len_str(std::ofstream& f, const std::string& s);
    static bool read_len_str(std::ifstream& f, std::string& out);
};
