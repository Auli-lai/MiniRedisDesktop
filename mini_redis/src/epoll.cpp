    ...
    while (offset < response.size()) {
        ...
        if (written == -1) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                ...
                conn->out_offset = 0;
                // 注册 EPOLLOUT 等待可写
                epoll->mod(fd, EPOLLIN | EPOLLOUT | EPOLLET | EPOLLONESHOT);
                return false;
            }
            Log::error("Write failed (fd=%d): %s", fd, strerror(errno));
            return false;
...
static void process_client_read(int fd, Epoll* epoll, Database& /*db*/,
    ...
    while (true) {
        ...
        if (res == RespParser::PARSE_OK) {
            ...
            if (!send_response(fd, epoll, conn, response)) {
                ...
                // EPOLLONESHOT 已在 send_response 中重新注册为 IN|OUT
                need_flush = true;
                break;
            }
    ...
    // ── 3. 重新注册事件 ──
    if (need_flush) {
        // 有未发送完的数据，保持 EPOLLIN|EPOLLOUT（已在 send_response 中设置）
        // 如果连接标记为 closing，下次 EPOLLOUT 时关闭
...
void Epoll::handle_events(int listen_fd, int nfds) {
    ...
    for (int i = 0; i < nfds; ++i) {
        ...
        if (fd == listen_fd) {
            ...
            // 可能同时有读和写事件（但 EPOLLONESHOT 通常只触发一种）
            if (ev & EPOLLOUT) {
