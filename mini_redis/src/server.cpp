...
void Server::start() {
    ...
    // 加载持久化数据（RDB 优先，不存在则尝试 AOF）
    g_db.set_rdb_file("dump.rdb");
    g_db.set_aof_file("appendonly.aof");
    if (!g_db.load_rdb()) {
        Log::info("No RDB file, trying AOF...");
        g_db.load_aof();
    }
    ...
    // 设置 listen_fd 为非阻塞，配合 ET accept 循环
    ...
    while (g_running) {
        ...
        // 处理事件
        epoll.handle_events(listen_fd, nfds);
