...
void Database::write_len_str(std::ofstream& f, const std::string& s) {
    f << s.size() << "\n";
    f.write(s.data(), s.size());
    f << "\n";
}

bool Database::read_len_str(std::ifstream& f, std::string& out) {
    std::string line;
    ...
    out.resize(len);
    f.read(&out[0], len);
    char c; f.get(c);  // 消费换行符
    return f.good() || f.eof();
}

bool Database::save_rdb() {
    std::lock_guard<std::mutex> lock(mtx);

    std::string filename = rdb_filename.empty() ? "dump.rdb" : rdb_filename;
    std::string tmpfile = filename + ".tmp";

    std::ofstream f(tmpfile, std::ios::binary);
    if (!f) {
        Log::error("RDB save: cannot open %s", tmpfile.c_str());
        return false;
    }
    ...
    for (const auto& kv : store) {
        auto eit = expires.find(kv.first);
        if (eit != expires.end() && now > eit->second) continue;
        liveKeys++;
    }
    ...
    for (const auto& kv : store) {
        ...
        RedisObject* obj = kv.second;

        auto eit = expires.find(key);
        if (eit != expires.end() && now > eit->second) continue;

        write_len_str(f, key);
        f << static_cast<int>(obj->type) << "\n";

        if (eit != expires.end()) f << eit->second << "\n";
        else                      f << "-1\n";

        switch (obj->type) {
        case TYPE_STRING:
            write_len_str(f, obj->str_val);
            break;
        case TYPE_LIST:
            f << obj->list_val.size() << "\n";
            for (const auto& elem : obj->list_val)
                write_len_str(f, elem);
            break;
        case TYPE_HASH:
            f << obj->hash_val.size() << "\n";
            for (const auto& kv2 : obj->hash_val) {
                write_len_str(f, kv2.first);
                write_len_str(f, kv2.second);
            }
            break;
        case TYPE_SET:
            f << obj->set_val.size() << "\n";
            for (const auto& member : obj->set_val)
                write_len_str(f, member);
            break;
        case TYPE_ZSET: {
            auto items = obj->zset_val
                ? obj->zset_val->range_with_scores(0, -1)
                : std::vector<std::pair<std::string, double>>();
            f << items.size() << "\n";
            for (const auto& kv2 : items) {
                write_len_str(f, kv2.first);
                f << kv2.second << "\n";
            }
            break;
    ...
    f.close();
    if (!f) { Log::error("RDB save: write error"); return false; }

    std::remove(filename.c_str());
    std::rename(tmpfile.c_str(), filename.c_str());

    Log::info("RDB saved: %d keys to %s", liveKeys, filename.c_str());
    return true;
}

bool Database::load_rdb() {
    std::lock_guard<std::mutex> lock(mtx);
    ...
    std::string header;
    if (!std::getline(f, header) || header != "MINIREDIS_RDB v1") return false;

    std::string tsLine, countLine;
    if (!std::getline(f, tsLine) || !std::getline(f, countLine)) return false;
    int keyCount = std::stoi(countLine);
    Log::info("RDB load: %d keys from %s", keyCount, filename.c_str());
    ...
    for (int i = 0; i < keyCount; ++i) {
        std::string key;
        if (!read_len_str(f, key)) return false;

        std::string typeLine;
        if (!std::getline(f, typeLine)) return false;
        ValueType type = static_cast<ValueType>(std::stoi(typeLine));

        std::string expireLine;
        if (!std::getline(f, expireLine)) return false;
        time_t expireTs = std::stoll(expireLine);

        RedisObject* obj = new RedisObject();
        obj->type = type;

        auto readCount = [&]() -> size_t {
            std::string cl; std::getline(f, cl); return std::stoull(cl);
        };

        switch (type) {
        case TYPE_STRING: {
            std::string val;
            if (!read_len_str(f, val)) { delete obj; return false; }
            obj->str_val = val;
            break;
        }
        case TYPE_LIST: {
            ...
            for (size_t j = 0; j < count; ++j) {
                std::string elem;
                if (!read_len_str(f, elem)) { delete obj; return false; }
                obj->list_val.push_back(elem);
            }
            break;
        }
        case TYPE_HASH: {
            ...
            for (size_t j = 0; j < count; ++j) {
                ...
                if (!read_len_str(f, field) || !read_len_str(f, val))
                    { delete obj; return false; }
                obj->hash_val[field] = val;
            }
            break;
        }
        case TYPE_SET: {
            ...
            for (size_t j = 0; j < count; ++j) {
                std::string member;
                if (!read_len_str(f, member)) { delete obj; return false; }
                obj->set_val.insert(member);
            }
            break;
        }
        case TYPE_ZSET: {
            obj->zset_val = new SkipList();
            size_t count = readCount();
            for (size_t j = 0; j < count; ++j) {
                std::string member, scoreLine;
                if (!read_len_str(f, member)) { delete obj; return false; }
                if (!std::getline(f, scoreLine)) { delete obj; return false; }
                obj->zset_val->insert(member, std::stod(scoreLine));
            }
            break;
        }
        }
        ...
        store[key] = obj;
        if (expireTs > 0) expires[key] = expireTs;
    }

    Log::info("RDB loaded: %d keys restored", (int)store.size());
    return true;
}
